...
class PrestaShopWebservice
{
	...
	{
		echo '<div style="display:table;background:#CCC;font-size:8pt;padding:7px"><h6 style="font-size:9pt;margin:0">' . $title . '</h6><pre>' . htmlentities($content) . '</pre></div>'; /*display:table*/
	}
	...
	public function get($options)
	{
		...
		elseif (isset($options['resource'])) {
			...
			if (count($url_params) > 0)
				$url .= '?' . http_build_query($url_params) . '&time=' . round(microtime(true) * 1000);
		} else
			throw new PrestaShopWebserviceException('Bad parameters given');
