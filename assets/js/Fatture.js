...
$(document).on("click", ".CreaNuovaFattura_fa", function () {
  CaricaFrame(
    currentURL + "contabilita/fattura.php",
    '<i class="fa-regular fa-file-invoice"></i> Crea fattura',
    "Crea una fattura emessa da un tuo fornitore!",
    "50%"
  );
});
...
$(document).on("click", ".NuovoPagamento_fa", function () {
  window.open(
    currentURL + "contabilita/carica.php",
    "Carica nuovo",
    "width=800, height=800, resizable, status, scrollbars=1, location"
  );
});
...
function AggiornaFatture_fa() {
  ...
  $.post(
    currentURL + "assets/inc/fatture.php",
    {
      azione: "aggiorna",
      statopag: statopag,
      clausola: clausola,
    ...
    function (data) {
      ...
      $("#FootTabellaFatture").html(
        '<tr><th colspan="4" class="text-end">€ ' +
          res[1] +
          "</th><th></th></tr>"
      );
      docReady(listInit);
...
function ControllaDataPagamento_fa() {
  ...
  } else if (radiosdate == "da") Aggiungicampi_fa();
}
...
async function NuovoPagamento_fa() {
  ...
  if (value == "nuovo") {
    ...
    if (nuovopag) {
      $.post(
        currentURL + "assets/inc/documenti.php",
        { azione: "nuovotipopagamento", dato: nuovopag },
        function (response) {
          if (response == "no") {
            Toast.fire({
              icon: "error",
              title:
                "Errore nel caricamento, si prega di controllare i dati inseriti",
            });
          } else {
            Toast.fire({
              icon: "success",
              title: "Nuovo tipo di pagamento salvato con successo",
            });
...
$(document).on("click", ".NuovoDocumento_fa", function () {
  if ($("#fa_fornitore").val() == "") {
    Swal.fire({
      icon: "error",
      title: "Inserisci il fornitore prima di procedere",
    });
  } else if ($("#ndoc_fatt").val() == "") {
    Swal.fire({
      icon: "error",
      title: "Inserisci il numero documento prima di procedere",
    });
  } else if ($("#fa_datadoc").val() == "") {
    Swal.fire({
      icon: "error",
      title: "Inserisci la data documento prima di procedere",
    });
  } else if ($("#fa_tippag").val() == "-") {
    Swal.fire({
      icon: "error",
      title: "Inserisci un metodo di pagamento prima di procedere",
    });
  } else if ($("#totalefattura").html() == "") {
    Swal.fire({
      icon: "error",
      title: "Devi riempire almeno un campo importo prima di procedere",
    });
  } else {
    bloccaui();
    var fornitore = parseInt(
      $("#fa_fornitore")
        .val()
        .replace(/[^0-9\.]/g, ""),
      10
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response == "si") {
          Toast.fire({
            icon: "success",
            title: "Fattura caricata correttamente!",
          });
          svuotacampi_fa();
        } else {
          Toast.fire({ icon: "error", title: "Errore: " + response });
...
$(document).on("click", ".ApriFattura_fa", function () {
  CaricaFrame(
    currentURL + "contabilita/fattura.php",
    '<i class="fa-regular fa-file-invoice"></i> Visualizza/Modifica fattura',
    "Visualizza o modifica una fattura emessa da un tuo fornitore!",
    "50%"
  );
...
function ApriFattura(id) {
  ...
  $.post(
    ...
    function (data) {
      ...
      $("#totale22").val(res[3]);
      $("#imp22").val(
        Math.round(($("#totale22").val() * 100) / 122).toFixed(2)
      );

      $("#totale10").val(res[4]);
      $("#imp10").val(
        Math.round(($("#totale10").val() * 100) / 122).toFixed(2)
      );
      ...
      $("#totaleese").val(res[6]);
      $("#impese").val(
        Math.round(($("#totaleese").val() * 100) / 122).toFixed(2)
      );

      $("#note").val(res[7]);
      $('#fa_tippag option[value="' + res[8] + '"]').attr(
        "selected",
        "selected"
      );
...
$(document).on("click", ".VisualizzaAllegato_fa", function () {
  ...
  window.open(
    currentURL + "upload/fatture/" + filename,
    "Visualizza",
    "width=800, height=800, resizable, status, scrollbars=1, location"
  );
});
  ...
  if ($("#idfatt").val() != "0") {
    ...
    $.post(
      currentURL + "assets/inc/fatture.php",
      {
        ...
        valore: $("#" + campo).val(),
        idfatt: $("#idfatt").val(),
        totale: $("#totalefattura").html(),
      },
      function (data) {
        if (data == "ok") {
          Toast.fire({
            icon: "success",
            title: "Campo aggiornato con successo!",
          });
          sbloccaui();
        } else {
...
$(document).on("click", ".eliminafatt", function () {
  ...
  $.post(
    currentURL + "assets/inc/fatture.php",
    { azione: "elimina", id: id },
    function (data) {
      if (data == "si") {
        Toast.fire({
          icon: "success",
          title: "Fattura eliminata con successo",
        });
        AggiornaFatture_fa();
      } else {
        Toast.fire({
          icon: "error",
          title: "Errore durante l'eliminazione della fattura. Errore:" + data,
        });
      }
      sbloccaui();
    }
